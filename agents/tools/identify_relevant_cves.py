...
from utilities.embedding_search import search_cev_embedding
...
class IdentifyRelevantCVEsTool(BaseTool):
    ...
    def _run(self, raw_incident: str) -> str:
        try:
            ...
            top_cves = search_cev_embedding(query=incident.model_dump_json())
            if not top_cves:
                return "No relevant CVEs found."

            cve_block = "\n\n".join(
                [
                    f"CVE #{i + 1} (score={score:.4f}):\n{cve}"
                    for i, (cve, score) in enumerate(top_cves)
                ],
            )
...
And here are candidate CVEs retrieved from a semantic search:
{cve_block}

From these, select the most relevant CVEs (1 to 3)
that are likely to be associated with this incident.

**IMPORTANT**:
- Analyze the relationship between the observed incident context and the retrieved CVEs.
...
"""
            ...
            msg = f"IdentifyRelevantCVEsTool failed: {e}"
            logger.warning(
                msg,
                exc_info=True,
            )
            return f"Unable to identify relevant CVEs due to an error: {e}"

        logger.warning("Unexpected LLM response format")
        return "Failed to identify relevant CVEs."
